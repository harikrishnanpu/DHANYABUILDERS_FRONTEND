...
const useMapLocations = (setMapCenter, setZoom) => {
            ...
            if (key.startsWith('lastlocation')) {
                try {
                    ...
                    if (storedData?.latitude && storedData?.longitude) {
                        storedLocations[userId] = {
                            ...storedData,
                            path: storedData.path || [], // Initialize path if not present
                        };
                    }
                } catch (error) {
    ...
    // Update a user's location with a new path
    socket.on('location-updated', (newLocation) => {
        setLocations((prevLocations) => {
            const updatedLocations = {
                ...prevLocations,
                [newLocation.userId]: {
                    ...prevLocations[newLocation.userId], // Spread existing data
                    longitude: newLocation.longitude,
                    latitude: newLocation.latitude,
                    name: newLocation.userName,
                    icon: newLocation.iconUrl,
                    path: prevLocations[newLocation.userId]?.path ? [
                        ...prevLocations[newLocation.userId].path, 
                        [newLocation.latitude, newLocation.longitude]
                    ] : [[newLocation.latitude, newLocation.longitude]], // Ensure path exists
                },
            };
                ...
                >
                    {isMapLoaded &&
                        Object.entries(locations).map(([id, loc]) => {
                            if (!loc) return null; // Skip if loc is undefined

                            return (
                                <React.Fragment key={id}>
                                    {/* Draw the path for each user */}
                                    <Polyline
